...
void setup() {
  ...
  while (!gpsSynced) {
    ...
    snprintf(filename, 32, "%d_%d_%d_%d_%d_%d", date[0], date[1], date[2], date[3], date[4], date[5]);
    lcd.setCursor(0, 150);
  ...
  if (!f) {
    setCursor_Text_Delay(lcd, 0, 170, "[NOK]\t| SD: n'as pas pu créer le fichier.            .", 250);
    return ((void)Serial.println("Erreur: n'as pas pu créer le fichier"));
  }
  setCursor_Text_Delay(lcd, 0, 170, "[OK]\t| SD: File opened.            .", 250);
